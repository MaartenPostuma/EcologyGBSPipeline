...
rule all:
    input:
        ...
        R1=expand("{path}/demultiplex/samples/{{RUNSAMPLE}}_R1.fq.gz",path=config["outputDir"]),
        R2=expand("{path}/demultiplex/samples/{{RUNSAMPLE}}_R2.fq.gz",path=config["outputDir"])
