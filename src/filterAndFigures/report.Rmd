...
print(ggplot(pcaDataSub,aes(x=PCA1,y=PCA2,col=metaPop,label=pop))+
  ...
  ggtitle(paste0("PCA Max missing = ",max_missing," maf= ",maf," nSNP=",pcaData$nSNPs)))
...
for(i in 1:length(unique(parameters$maf))){
 ...
 treeSegments<-treeSegments[treeSegments$max_missing==max_missing&treeSegments$maf==maf,]
 treeLabels<-treeLabels[treeLabels$max_missing==max_missing&treeLabels$maf==maf,]

 print(ggplot(treeSegments) +
   geom_segment(aes(x = x, y = y, xend = xend, yend=yend))+
   theme_pubr()+coord_cartesian(ylim=c(-3,max(treeSegments$y)*1.15))+
   geom_text(data = treeLabels, aes(x, y, label = label,col=metaPop),
             hjust = 1, angle = 90, size = 5)+
   xlab("")+
   ylab("genetic distance")+
   ggtitle(paste("number of SNPs =",unique(treeLabels$nSNPs),"\n maxMissing =",max_missing,"\n maf =",maf)))
}
}
```
